...
export default function OfferPage() {
  ...
  if (loading) {
    return (
      <html lang="no">
        <body className="bg-[#0A0A0A] text-[#F5F0E8] font-sans antialiased min-h-screen flex items-center justify-center">
          <p className="text-[#6B6B6B]">Laster tilbud...</p>
        </body>
      </html>
    );
  }
      ...
      <html lang="no">
        <body className="bg-[#0A0A0A] text-[#F5F0E8] font-sans antialiased min-h-screen flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-[#6B6B6B]">Denne lenken er ugyldig eller tilbudet har utløpt.</p>
          </div>
        </body>
      </html>
    );
  ...
  if (offer.status !== "sent" && !done) {
    ...
    return (
      <html lang="no">
        <body className="bg-[#0A0A0A] text-[#F5F0E8] font-sans antialiased min-h-screen flex items-center justify-center px-6">
          <div className="text-center max-w-md">
            <h1 className="text-2xl tracking-[0.15em] uppercase font-semibold mb-8">Bar<span className="text-[#C9A84C]">Pro</span></h1>
            <p className="text-[#6B6B6B]">{statusText}</p>
          </div>
        </body>
      </html>
    );
  ...
  return (
    <html lang="no">
      <body className="bg-[#0A0A0A] text-[#F5F0E8] font-sans antialiased min-h-screen flex items-center justify-center px-6 py-12">
        <div className="w-full max-w-lg">
          {/* Logo */}
          <div className="text-center mb-10">
            <h1 className="text-2xl tracking-[0.15em] uppercase font-semibold">Bar<span className="text-[#C9A84C]">Pro</span></h1>
          </div>

          {done ? (
            <div className="border border-[#1E1E1E] bg-[#141414] p-10 text-center">
              ...
              {done === "accepted" ? (
                <>
                  ...
                  <p className="text-[#6B6B6B]">Takk, {offer.booking.customer_name}! Vi gleder oss til arrangementet ditt. Vi tar kontakt med detaljer.</p>
                </>
              ) : (
                <>
            ...
            <div className="border border-[#1E1E1E] bg-[#141414]">
              {/* Header */}
              <div className="p-8 border-b border-[#1E1E1E] text-center">
                <p className="text-[10px] text-[#6B6B6B] uppercase tracking-[0.25em] mb-2">Tilbud til</p>
              ...
              <div className="p-8 space-y-4 text-sm">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  {offer.booking.start_time && (
                    <div>
                      <p className="text-[10px] text-[#6B6B6B] uppercase tracking-wider mb-1">Tid</p>
                ...
                {offer.notes && (
                  <div className="border-t border-[#1E1E1E] pt-4">
                    <p className="text-[10px] text-[#6B6B6B] uppercase tracking-wider mb-1">Detaljer</p>
                    <p className="text-[#6B6B6B]">{offer.notes}</p>
                  </div>
                )}
              </div>

              {/* Price */}
              <div className="border-t border-[#1E1E1E] p-8 text-center bg-[#C9A84C]/[0.04]">
                <p className="text-[10px] text-[#6B6B6B] uppercase tracking-[0.25em] mb-2">Totalpris</p>
                <p className="text-4xl font-semibold text-[#C9A84C]">{offer.offered_price.toLocaleString("no-NO")} kr</p>
                <p className="text-[11px] text-[#6B6B6B] mt-1">Ekskl. mva</p>
              </div>
                ...
                {showRejectForm ? (
                  <div className="space-y-4">
                    ...
                    <label className="flex items-center gap-3 cursor-pointer">
                      <input type="checkbox" checked={wantsNewOffer} onChange={(e) => setWantsNewOffer(e.target.checked)} className="accent-[#C9A84C]" />
                      <span className="text-sm text-[#6B6B6B]">Jeg ønsker et nytt tilbud</span>
                    </label>
                    <div className="flex gap-3">
                      <button onClick={() => setShowRejectForm(false)}
                        className="flex-1 border border-[#1E1E1E] py-3 text-xs text-[#6B6B6B] uppercase tracking-wider hover:text-[#F5F0E8] cursor-pointer">
                        Tilbake
                      </button>
          ...
          <p className="text-center text-[10px] text-[#6B6B6B]/40 mt-8 uppercase tracking-wider">&copy; 2026 BarPro</p>
        </div>
      </body>
    </html>
  );
