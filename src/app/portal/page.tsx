...
export default function PortalPage() {
  ...
  if (!employee) return (
    <div className="space-y-4">
      <div className="h-8 w-48 bg-[#1A1410] animate-pulse" />
      <div className="grid grid-cols-3 gap-4">
        {[1,2,3].map((i) => <div key={i} className="h-24 bg-[#1A1410] animate-pulse" />)}
      </div>
      <div className="h-48 bg-[#1A1410] animate-pulse" />
    </div>
  );
    ...
    <div>
      ...
      {tab === "oversikt" && (
        <div className="space-y-6">
          <div className="grid grid-cols-3 gap-4">
            ...
            <div className="bg-[#1A1410] border border-[#1A1410] p-5">
              <p className="text-[10px] text-[#6B5D52] uppercase tracking-wider mb-1">Venter godkjenning</p>
          ...
          <div className="bg-[#1A1410] border border-[#1A1410] p-6">
            ...
            {upcoming.length === 0 ? (
              <p className="text-[#6B5D52] text-sm">Ingen kommende oppdrag</p>
            ) : upcoming.map((a) => (
              <div key={a.id} className="flex items-center justify-between py-3 border-b border-[#1A1410] last:border-0">
                <div>
          ...
          <div className="bg-[#1A1410] border border-[#1A1410] p-6">
            ...
            {timeEntries.length === 0 ? (
              <p className="text-[#6B5D52] text-sm">Ingen registrerte timer</p>
            ) : timeEntries.slice(0, 10).map((e) => (
              <div key={e.id} className="flex items-center justify-between py-2 border-b border-[#1A1410] last:border-0 text-sm">
                <div>
                  <p>{new Date(e.date + "T00:00:00").toLocaleDateString("no-NO", { day: "numeric", month: "short" })}</p>
                  <p className="text-[10px] text-[#6B5D52]">{e.description || "Ingen beskrivelse"}</p>
                </div>
      ...
      {tab === "kalender" && (
        <div>
          <div className="flex items-center justify-between mb-6">
            <button onClick={() => setCalMonth(new Date(calYear, calMo - 1))} className="text-[#6B5D52] hover:text-[#E8DDD4] cursor-pointer px-3 py-1">&larr;</button>
            <h2 className="text-lg font-medium">{monthNames[calMo]} {calYear}</h2>
            <button onClick={() => setCalMonth(new Date(calYear, calMo + 1))} className="text-[#6B5D52] hover:text-[#E8DDD4] cursor-pointer px-3 py-1">&rarr;</button>
          </div>

          <p className="text-[11px] text-[#6B5D52] mb-4">Klikk på en dag for å markere at du ikke er tilgjengelig</p>

          <div className="bg-[#1A1410] border border-[#1A1410]">
            <div className="grid grid-cols-7 border-b border-[#1A1410]">
              {["Man","Tir","Ons","Tor","Fre","Lør","Søn"].map((d) => (
                <div key={d} className="p-2 text-center text-[10px] tracking-wider uppercase text-[#6B5D52]">{d}</div>
              ))}
            </div>
            <div className="grid grid-cols-7">
              {calDays.map((day, i) => {
                if (!day) return <div key={`e-${i}`} className="p-2 min-h-[70px] border-b border-r border-[#1A1410]" />;
          ...
          {blockModal && (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => setBlockModal(null)}>
              <div className="bg-[#1A1410] border border-[#1A1410] w-full max-w-sm p-6" onClick={(e) => e.stopPropagation()}>
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-sm font-medium">
                    {new Date(blockModal.dateStr + "T00:00:00").toLocaleDateString("no-NO", { weekday: "long", day: "numeric", month: "long" })}
                  </h3>
                  <button onClick={() => setBlockModal(null)} className="text-[#6B5D52] hover:text-[#E8DDD4] cursor-pointer">&times;</button>
                </div>
                  ...
                  <div className="space-y-4">
                    <div className="bg-red-400/10 border border-red-400/20 p-3">
                      ...
                      {blockModal.existing.reason && (
                        <p className="text-[11px] text-[#6B5D52] mt-1">Grunn: {blockModal.existing.reason}</p>
                      )}
                    </div>
                    <button onClick={confirmUnblock} disabled={saving}
                      className="w-full border border-[#1A1410] py-2 text-xs text-[#6B5D52] uppercase tracking-wider hover:text-[#E8DDD4] cursor-pointer disabled:opacity-50">
                      Fjern markering
                    </button>
                  </div>
                ) : (
                  <div className="space-y-4">
                    <p className="text-[11px] text-[#6B5D52]">Marker denne dagen som utilgjengelig. Admin vil se dette når de planlegger oppdrag.</p>
                    <div>
      ...
      {tab === "timer" && (
        <div className="max-w-lg">
          <div className="bg-[#1A1410] border border-[#1A1410] p-6 space-y-5">
            ...
            <div>
              <label className="text-[10px] text-[#6B5D52] uppercase tracking-wider mb-2 block">Tidspunkt</label>
          ...
          {/* Recent entries */}
          <div className="mt-6 bg-[#1A1410] border border-[#1A1410] p-6">
            <h2 className="text-sm font-medium mb-4">Dine registreringer</h2>
            {timeEntries.length === 0 ? (
              <p className="text-[#6B5D52] text-sm">Ingen registreringer</p>
            ) : timeEntries.map((e) => (
              <div key={e.id} className="flex items-center justify-between py-2 border-b border-[#1A1410] last:border-0 text-sm">
                <div>
          ...
          <div className="bg-[#1A1410] border border-[#1A1410] p-6">
            ...
            {editingProfile ? (
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  <p className="text-[10px] text-[#6B5D52] uppercase tracking-wider">E-post</p>
                  <p>{employee.email}</p>
                </div>
                <div>
                  <p className="text-[10px] text-[#6B5D52] uppercase tracking-wider">Telefon</p>
                  <p>{employee.phone || "—"}</p>
                </div>
                <div>
                  <p className="text-[10px] text-[#6B5D52] uppercase tracking-wider">Timelønn</p>
