  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-6 mb-6 text-[11px] text-[#6B6B6B]">
        <div className="flex items-center gap-2"><div className="w-3 h-3 bg-transparent border border-[#141414]" /> Ledig</div>
      ...
      <div className="bg-[#141414] border border-[#141414]">
        ...
        <div className="grid grid-cols-7">
          {days.map((day, i) => {
            if (!day) return <div key={`e-${i}`} className="p-3 min-h-[80px] border-b border-r border-[#141414]" />;
            ...
            return (
              <button key={day} onClick={() => openDayModal(day)}
                className={`p-3 min-h-[80px] border-b border-r border-[#141414] text-left transition-colors duration-200 cursor-pointer hover:bg-[#1A1A1A] ${bgClass} ${isPast ? "opacity-40" : ""}`}>
      ...
      {modal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" onClick={() => setModal(null)}>
          <div className="bg-[#141414] border border-[#141414] w-full max-w-lg max-h-[80vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            {/* Header */}
            ...
            <div className="p-6 space-y-6">
              ...
              {modal.booking ? (
                <div className="space-y-4">
                  ...
                  <div className="bg-[#0A0A0A] border border-[#141414] p-4 space-y-3">
                    <div className="flex items-center justify-between">
                    ...
                    <div>
                      ...
                      <div className="space-y-1">
                        {modal.assignments.map((a) => (
                          <div key={a.id} className="flex items-center justify-between py-1.5 px-2 bg-[#0A0A0A] border border-[#141414] text-sm">
                ...
                {modal.blocked ? (
                  <div className="space-y-3">
                    ...
                    <button onClick={unblockDate} disabled={saving}
                      className="w-full border border-[#141414] py-2 text-xs text-[#6B6B6B] uppercase tracking-wider hover:text-[#F5F0E8] transition-colors cursor-pointer disabled:opacity-50">
                      Fjern blokkering
                    </button>
                  </div>
                ) : (
                  <div className="space-y-3">
                    <input value={blockReason} onChange={(e) => setBlockReason(e.target.value)}
                      ...
                      className="w-full bg-[#0A0A0A] border border-[#141414] px-3 py-2 text-sm outline-none focus:border-[#C9A84C]/40 placeholder:text-[#6B6B6B]/40" />
